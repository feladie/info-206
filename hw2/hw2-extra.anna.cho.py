...
# Check if user password is in 'common.txt':
	try:
		in_common(password, common_list(), 0, len(common_list()) - 1, (len(common_list()) // 2) - 1, 1)
	except:
...
def in_common(password, lst, minimum, maximum, mid, count):
...
# Use the binary search algorithm to search whether the user's password is in the list of commont passwords:
	...
	if maximum - minimum <= 1:
		if lst[minimum] == lowercase_password:
			warning += '\nNumber of binary search comparisons: {}'.format(count)
		elif lst[maximum] == lowercase_password:
			warning += '\nNumber of binary search comparisons: {}'.format(count + 1)
		else: 
			warning += '\nNumber of binary search comparisons: {}'.format(count + 1)
		print(warning)
		return warning
	elif lst[mid] == lowercase_password:
		warning += '\nNumber of binary search comparisons: {}'.format(count)
		print(warning)
		return warning
	elif lowercase_password < lst[mid]:
		new_max = mid - 1
		new_mid = minimum + (new_max - minimum) // 2
		return in_common(password, lst, minimum, new_max, new_mid, count + 1)
	elif lowercase_password > lst[mid]:
		new_min = mid + 1
		new_mid = new_min + (maximum - new_min) // 2
		return in_common(password, lst, new_min, maximum, new_mid, count + 1)
